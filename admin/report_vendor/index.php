...
$qry = $conn->query("SELECT * FROM `accounts` WHERE date(date_created) BETWEEN '$date_from' AND '$date_to' ORDER BY date(date_created) DESC");
    ...
    $items = $conn->query("SELECT * FROM invoices_items where invoice_id = {$row['id']} ")->num_rows;
					...
					?>
						<tr>
							...
							<td class="align-middle py-1 px-2"><?php echo $row['account_number'] ?></td>
							<td class="align-middle py-1 px-2"><?php echo $row['firstname'] ?></td>
							<td class="align-middle py-1 px-2">
								<dl class="lh-1">
									<dt class="my-0 py-0 text-info">Email:</dt>
									<dd class="my-0 py-0 pl-3"><?php echo $row['email'] ?></dd>
									<dt class="my-0 py-0 text-info">Mobile #:</dt>
									<dd class="my-0 py-0 pl-3"><?php echo $row['balance'] ?></dd>
								</dl>
							</td>
							<td class="text-right align-middle py-1 px-2"><?php echo $row['address'] ?></td>
						</tr>
					<?php endwhile; ?>
				</tbody>
			</table>
