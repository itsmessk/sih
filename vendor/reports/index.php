...
<div class="card card-outline card-primary">
	<div class="card-header">
		<h3 class="card-title">Procurement Reports</h3>
...
$qry = $conn->query("SELECT * FROM `invoice_list` WHERE date(date_created) BETWEEN '$date_from' AND '$date_to' ORDER BY date(date_created) DESC");
    ...
    $items = $conn->query("SELECT * FROM invoices_items where invoice_id = {$row['id']} ")->num_rows;
					...
					?>
						<tr>
							...
							<td class="align-middle py-1 px-2"><?php echo $row['invoice_code'] ?></td>
							<td class="align-middle py-1 px-2"><?php echo $row['customer_name'] ?></td>
							<td class="align-middle py-1 px-2">
								<dl class="lh-1">
									<dt class="my-0 py-0 text-info">Invoice Type:</dt>
									<dd class="my-0 py-0 pl-3"><?php echo $row['type'] == 1 ? "Product":"Service" ?></dd>
									<dt class="my-0 py-0 text-info">Item Count:</dt>
									<dd class="my-0 py-0 pl-3"><?php echo number_format($items) ?></dd>
								</dl>
							</td>
							<td class="text-right align-middle py-1 px-2"><?php echo number_format($row['total_amount'],2) ?></td>
						</tr>
					<?php endwhile; ?>
				</tbody>
			</table>
